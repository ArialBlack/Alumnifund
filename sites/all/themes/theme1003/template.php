...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme1003
 */

// Auto-rebuild the theme registry during theme development.
if ( theme_get_setting( 'theme1003_clear_registry' ) ) {
	// Rebuild .info data.
 ...
 */
function theme1003_html_head_alter( &$head_elements ) {
 ...
 */
function theme1003_preprocess_search_block_form( &$vars ) {
 ...
 */
function theme1003_preprocess( &$vars, $hook ) {
	$vars['theme1003_path'] = base_path() . path_to_theme();
 ...
 */
function theme1003_preprocess_html( &$vars ) {
	$vars['doctype'] = _theme1003_doctype();
	$vars['rdf'] = _theme1003_rdf( $vars );
	...
	if ( !$vars['is_front'] ) {
		...
		if ( isset( $page_name ) ) {
			$vars['classes_array'][] = theme1003_id_safe( 'page-' . $page_name );
		}

		$vars['classes_array'][] = theme1003_id_safe( 'section-' . $section );
	...
	if ( ( $d['destination'] == 'blog' ) && ( theme_get_setting( 'theme1003_blog_title' ) != "" ) ) {
		$vars['title'] = theme_get_setting( 'theme1003_blog_title' );
		$vars['head_title'] = theme_get_setting( 'theme1003_blog_title' );
	}
}
 ...
 */
function theme1003_preprocess_page( &$vars ) {
 ...
 */
function theme1003_preprocess_maintenance_page( &$vars ) {
	...
	drupal_add_css( drupal_get_path( 'theme', 'theme1003' ) . '/css/maintenance-page.css' );
}
 ...
 */
function theme1003_preprocess_node( &$vars ) {
	// Striping class
 ...
 */
function theme1003_preprocess_block( &$vars, $hook ) {
 ...
 */
function theme1003_menu_tree( $vars ) {
 ...
 */
function theme1003_field__taxonomy_term_reference( $vars ) {
	$output = '';
 ...
 */
function theme1003_breadcrumb( $vars ) {
	$breadcrumb = isset( $vars['breadcrumb'] ) ? $vars['breadcrumb'] : array();

	$condition = theme_get_setting( 'theme1003_breadcrumb_hideonlyfront' ) ? count( $breadcrumb ) > 1
			: !empty( $breadcrumb );
	$separator = theme_get_setting( 'theme1003_breadcrumb_separator' );

	if ( theme_get_setting( 'theme1003_breadcrumb_showtitle' ) ) {
 ...
 * @return bool
 */
function theme1003_tabs_float() {
	$float = (bool) theme_get_setting( 'theme1003_tabs_float' );
	$float_node = (bool) theme_get_setting( 'theme1003_tabs_node' );
...
function theme1003_id_safe( $string ) {
	// Strip accents
 ...
 * @ingroup themeable
 */
function theme1003_menu_link( array $vars ) {
	...
	$element['#attributes']['class'][] = theme1003_id_safe( $element['#title'] );
 ...
 */
function theme1003_preprocess_menu_local_task( &$vars ) {
 ...
 */
function theme1003_menu_local_tasks( &$vars ) {
	$output = '';
 ...
 */
function theme1003_preprocess_comment( &$variables ) {
...
/* Region options */
function theme1003_region_preffix ( $reg ) {
	$block_bg_type = theme_get_setting( 'theme1003_' . $reg . '_bg_type' );
	$block_bg_img = theme_get_setting( 'theme1003_' . $reg . '_bg_img' );
	$block_bg_parallax = theme_get_setting( 'theme1003_' . $reg . '_bg_parallax' );
	$bg_video = theme_get_setting( 'theme1003_' . $reg . '_bg_video' );
	$bg_video_start = theme_get_setting( 'theme1003_' . $reg . '_bg_video_start' );
	$fullwidth = theme_get_setting( 'theme1003_' . $reg . '_fullwidth' );
...
function theme1003_region_suffix ( $reg ) {
	$fullwidth = theme_get_setting( 'theme1003_' . $reg . '_fullwidth' );
	$block_bg_parallax = theme_get_setting( 'theme1003_' . $reg . '_bg_parallax' );
	$block_bg_img = theme_get_setting( 'theme1003_' . $reg . '_bg_img' );
...
function theme1003_form_contact_site_form_alter( &$form, &$form_state, $form_id ) {
